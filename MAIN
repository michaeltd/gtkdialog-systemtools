...
<action>dmesg > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>
...
<action>dmesg | grep ${VAR2} > ${TMP_FILE} && ${EDITOR} ${TMP_FILE} </action>
</button>
...
<button>
<label>Help</label>
<action>${VAR1} --help > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button>
<label>Whereis</label>
<action>whereis ${VAR1} > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button>
<label>Which</label>
<action>which ${VAR1} > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button>
<label>Version</label>
<action>${VAR1} --version > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button>
<label>Manual</label>
<action>man ${VAR1} > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>
</hbox>
</frame>
<hbox homogeneous="True">
...
<label>Sensors</label>
<action>sensors > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button homogeneous="true">
<input file stock="gtk-connect"></input>
<label>Ethernet Interfaces</label>
<action>ifconfig > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button homogeneous="true">
<input file stock="gtk-disconnect"></input>
<label>Wireless Interfaces</label>
<action>iwconfig > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>
...
<action>sudo -A lshw > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button homogeneous="true">
<input file stock="gtk-print-preview"></input>
<label>BIOS information</label>
<action>sudo -A dmidecode | head -15 > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button homogeneous="true">
<input file stock="gtk-page-setup"></input>
<label>PCI devices</label>
<action>lspci > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>
</vbox>
</frame>
...
<action>uname -a > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>Show Path</label>
<action>echo "${PATH}" > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
...
<action>sudo -A cat /var/lib/portage/world &> ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
<menuitem>
<label>List Installed Ebuilds</label>
<action>cd /var/db/pkg/ && /bin/ls -d1 */* > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
<menuitem>
<label>Search Installed Ebuilds</label>
<action>${GTKDIALOG} --center --program=SEARCH > ${TMP_FILE} && source ${TMP_FILE} && [[ "${EXIT}" == "OK" ]] && cd /var/db/pkg/ && /bin/ls -d1 */* | grep ${SEARCH} > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
<menuitem>
<label>Search Portage tree</label>
<action>${GTKDIALOG} --center --program=SEARCH > ${TMP_FILE} && source ${TMP_FILE} && [[ "${EXIT}" == "OK" ]] && '"${TERMINAL[@]}"' "emerge -S ${SEARCH}"</action>
</menuitem>
...
<action>sudo -A fdisk -l > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>Hard drive UUID</label>
<action>blkid > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>Current mount points</label>
<action>mount > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>Available disk space</label>
<action>df -h > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>Connected USB devices</label>
<action>lsusb > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
...
<action>xdpyinfo > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>

<menuitem>
<label>GLX/OpenGL Information</label>
<action>glxinfo > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
...
<action>groups > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</menuitem>
...
<action>lsmod > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>

<button>
<label>Services</label>
<action>rc-update > ${TMP_FILE} && ${EDITOR} ${TMP_FILE}</action>
</button>
...
<action>${GTKDIALOG} --center --program=INSTALL > ${TMP_FILE} && source ${TMP_FILE} && [[ "${EXIT}" == "OK" ]] && sudo -A '"${TERMINAL[@]}"' "emerge --ask mesa-progs xdpyinfo lm-sensors dmidecode lshw usbutils"</action>
</button>
